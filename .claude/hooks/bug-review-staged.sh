...
# Auto-splits large staged changes into batches for reliable reviews
...
REVIEW_TMPFILE=""
start_spinner() {
  local label="${1:-Bug review: ${FILE_COUNT} staged files, ~${DIFF_LINES} lines...}"
  local frames=('⠋' '⠙' '⠹' '⠸' '⠼' '⠴' '⠦' '⠧' '⠇' '⠏')
...
# --- Single review (small staged changes) ---
run_single_review() {
  local files="$1"
  local file_list
...
# --- Batch review for a subset of staged files ---
run_batch_review() {
  local batch_num="$1"
  ...
  local scoped_diff="git diff --cached -- $batch_files"
...
run_summary() {
  ...
  cat > "$PROMPT_FILE" <<EOF
You are a senior code reviewer. Multiple batches of staged files have been reviewed independently. Combine the results into a single coherent review.
...
if [ "$FILE_COUNT" -le "$BATCH_SIZE" ]; then
  ...
  run_single_review "$STAGED_FILES" > "$REVIEW_TMPFILE" &
  claude_pid=$!
  wait "$claude_pid" 2>/dev/null || true
  claude_pid=""
  ...
  # Large change — split into batches
  TOTAL_BATCHES=$(( (FILE_COUNT + BATCH_SIZE - 1) / BATCH_SIZE ))

  BATCH_LIST_DIR=$(mktemp -d)
  echo "$STAGED_FILES" | awk -v size="$BATCH_SIZE" -v dir="$BATCH_LIST_DIR" '
    BEGIN { batch = 1 }
