...
COMMIT="${1:-HEAD}"
BATCH_SIZE="${REVIEW_BATCH_SIZE:-10}"  # files per batch, override with env var
...
start_spinner() {
  ...
  while true; do
    ...
    for frame in "${frames[@]}"; do
      printf "\r  ${frame} ${label} %dm%02ds elapsed (Ctrl+C to cancel)" "$mins" "$secs" >&2
      sleep 0.1
...
# --- Single batch review (small commits) ---
run_single_review() {
  local files="$1"
  local file_list
...
run_batch_review() {
  ...
  # Build a diff command scoped to just these files
...
# --- Summary pass: combine all batch results into a final verdict ---
run_summary() {
  local all_results="$1"

  cat > "$PROMPT_FILE" <<EOF
You are a senior code reviewer. Multiple batches of files from the same commit have been reviewed independently. Your job is to combine the results into a single coherent review.
...
if [ "$FILE_COUNT" -le "$BATCH_SIZE" ]; then
  ...
  start_spinner "Reviewing ${FILE_COUNT} files..." &
  spin_pid=$!
  ...
  echo "Large commit detected: splitting into ${TOTAL_BATCHES} batches of ~${BATCH_SIZE} files"
  echo ""

  # Split files into a temp directory, one file per batch
  ...
  for batch_file in "$BATCH_LIST_DIR"/batch_*.txt; do
    ...
    echo "━━━ Batch ${BATCH_NUM}/${TOTAL_BATCHES} (${batch_file_count} files) ━━━"
    sed 's/^/  - /' "$batch_file"

    start_spinner "Batch ${BATCH_NUM}/${TOTAL_BATCHES}..." &
    spin_pid=$!
...
${batch_result}"
  ...
  start_spinner "Generating final verdict..." &
  spin_pid=$!
